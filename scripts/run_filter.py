...
att_ax[0].plot(r2d(df0_nav["phi_rad"]), color="g", label="On Board")
att_ax[0].plot(r2d(df1_nav["phi_rad"]), label=filter.name)
att_ax[0].grid()

att_ax[1].set_ylabel("Pitch (deg)", weight="bold")
att_ax[1].plot(r2d(df0_nav["the_rad"]), color="g", label="On Board")
att_ax[1].plot(r2d(df1_nav["the_rad"]), label=filter.name)
att_ax[1].grid()

att_ax[2].set_ylabel("Yaw (deg)", weight="bold")
att_ax[2].plot(r2d(df0_nav["psi_rad"]), color="g", label="On Board")
att_ax[2].plot(r2d(df1_nav["psi_rad"]), label=filter.name)
...
ax1.plot(df0_nav["vn_mps"], label="On Board")
ax1.plot(df1_nav["vn_mps"], label=filter.name)
ax1.grid()

# ve Plot
ax2.set_ylabel("ve (mps)", weight="bold")
ax2.plot(df0_gps["ve_mps"], "-*", label="GPS Sensor", c="g", alpha=.5)
ax2.plot(df0_nav["ve_mps"], label="On Board")
ax2.plot(df1_nav["ve_mps"], label=filter.name)
ax2.grid()

# vd Plot
ax3.set_ylabel("vd (mps)", weight="bold")
ax3.plot(df0_gps["vd_mps"], "-*", label="GPS Sensor", c="g", alpha=.5)
ax3.plot(df0_nav["vd_mps"], label="On Board")
ax3.plot(df1_nav["vd_mps"], label=filter.name)
...
plt.plot(df1_nav["alt_m"], label=filter.name)
...
plt.plot(r2d(df1_nav["lon_rad"]), r2d(df1_nav["lat_rad"]), label=filter.name)
plt.grid()
plt.legend(loc=0)
ax = plt.gca()
ax.axis("equal")
