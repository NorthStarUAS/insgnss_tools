...
MATRIX mat_add(MATRIX A, MATRIX B, MATRIX C)
{
	...
	if (MatRow(C) != MatRow(A) || MatCol(C) != MatCol(B) || MatRow(B) != MatRow(A) || MatCol(A) != MatCol(B)) {
		printf("mat_add error: incompatible matrix size\n");
	//	_exit(-1);
...
MATRIX mat_copy(MATRIX A, MATRIX C)
{
	...
	if ( MatRow(A) != MatRow(C) || MatCol(A) != MatCol(C) ) {
		printf("mat_copy error: incompatible matrix size\n");
	//	_exit(-1);
	// if dimensions of C is correct
	} else {
		for (i=0; i<MatRow(A); i++)
...
//MATRIX A;
{
	if (A==NULL)
    	...
    	return(mat_fdumpf(A, "%.18f ", stdout));
}
...
MATRIX mat_inv(MATRIX a, MATRIX C)
{
	...
	// if dimensions of C is wrong
	if ( MatRow(a) != MatRow(C) || MatCol(a) != MatCol(C) ) {
		printf("mat_inv error: incompatible matrix size\n");
		_exit(-1);
	// if dimensions of C is correct
	} else {
		/*
...
MATRIX mat_mul(MATRIX A,MATRIX B, MATRIX C)
{
	...
	// if dimensions of C is wrong
	if (MatRow(C) != MatRow(A) || MatCol(C) != MatCol(B) || MatCol(A) != MatRow(B)) {
		printf("mat_mul error: incompatible matrix size\n");
	//	_exit(-1);
	// if dimensions of C is correct
	} else {
...
/* computes C = A * B' */
	...
	int		i, j, k;
	// if dimensions of C is wrong
	if (MatRow(C) != MatRow(A) || MatCol(C) != MatRow(B) || MatCol(A) != MatCol(B)) {
		printf("mat_transmul error: incompatible matrix size\n");
		//	_exit(-1);
		// if dimensions of C is correct
	}
	else {
...
MATRIX mat_sub(MATRIX A, MATRIX B, MATRIX C)
{
	...
	// if dimensions of C is wrong
	if (MatRow(A) != MatRow(C) || MatCol(A) != MatCol(C) || MatCol(A) != MatCol(B) || MatRow(A) != MatRow(B)) {
		printf("mat_sub error: incompatible matrix size\n");
	//	_exit(-1);
	// if dimensions of C is correct
	} else {
...
MATRIX mat_round (MATRIX X, MATRIX C)
{
	...
	if (  MatRow(C) != MatRow(X) ||  MatCol(C) != MatCol(X)  ) {
		printf("mat_round error: incompatible matrix size\n");
		_exit(-1);
	// if dimensions of C is correct
	} else {
...
MATRIX mat_scalMult (MATRIX X,double A, MATRIX C)
{
	...
	// if dimensions of C is wrong
	if (  MatRow(C) != MatRow(X) ||  MatCol(C) != MatCol(X)  ) {
		printf("mat_scalMult error: incompatible matrix size\n");
	//	_exit(-1);
	// if dimensions of C is correct
	} else {
